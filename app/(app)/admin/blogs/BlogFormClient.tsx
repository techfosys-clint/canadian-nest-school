...
export default function BlogFormClient({ initialBlog }: BlogFormClientProps) {
  ...
  const [metaDescription, setMetaDescription] = useState(
    initialBlog?.seo?.metaDescription || '',
  );
    ...
    try {
      ...
      await Swal.fire({
        ...
        text: initialBlog?.id
          ...
          : 'The article is now live on the platform.',
        timer: 1800,
        showConfirmButton: false,
        background: '#ffffff',
        color: '#1a1a1a',
      ...
      <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200/80 pb-6'>
        <div className='space-y-1'>
          ...
          <p className='text-base font-semibold text-slate-500 pl-13'>
            Construct SEO-rich blog articles and product announcements for
      ...
      <div className='grid grid-cols-1 lg:grid-cols-12 gap-8'>
        ...
        <div className='lg:col-span-4 space-y-6'>
          ...
          <div className='bg-white border border-slate-200/60 rounded-lg p-6 space-y-4'>
            <h3 className='text-base font-bold text-slate-800 uppercase tracking-wider pb-2 border-b border-slate-200/50'>
              Cover Image
            </h3>
            ...
            {coverImageUrl ? (
              <div className='space-y-3'>
                ...
                <div className='grid grid-cols-2 gap-2'>
                  <label
                    className={`flex items-center justify-center gap-1.5 py-2.5 rounded-lg bg-slate-100/80 hover:bg-slate-100 border border-slate-300 text-slate-600 font-bold text-sm cursor-pointer transition-colors ${uploading ? 'opacity-50 pointer-events-none' : ''}`}
                  >
                    <input
                      type='file'
                      accept='image/*'
              ...
              <div className='space-y-3'>
                <label
                  className={`flex flex-col items-center justify-center aspect-video w-full bg-slate-100 border-2 border-dashed border-slate-200 hover:border-[#E61C24]/50 rounded-lg cursor-pointer transition-colors p-4 text-center ${uploading ? 'opacity-50 pointer-events-none' : ''}`}
                >
                  <input
                    type='file'
                    accept='image/*'
                  ...
                  <span className='text-base font-semibold text-slate-500'>
                    {uploading ? 'Uploading Cover...' : 'Upload cover photo'}
                  </span>
          ...
          <div className='bg-white border border-slate-200/60 rounded-lg p-6 space-y-4'>
            ...
            <div className='flex flex-col gap-2'>
              ...
              <p className='text-xs font-semibold text-slate-400 leading-normal'>
                Separate multiple article tags using comma characters. Tag
                labels are parsed automatically.
              </p>
            </div>
          </div>
