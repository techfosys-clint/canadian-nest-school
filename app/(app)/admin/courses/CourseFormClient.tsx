...
export default function CourseFormClient({
  ...
  const [categories, setCategories] = useState<string[]>(
    initialData?.categories?.map((c: any) => c._id || c) ||
      (initialData?.category
        ? [initialData.category._id || initialData.category]
        : []),
  );
  const [instructors, setInstructors] = useState<string[]>(
    initialData?.instructors?.map((i: any) => i._id || i) ||
      (initialData?.instructor
        ? [initialData.instructor._id || initialData.instructor]
  ...
  const [showMediaPickerForMaterial, setShowMediaPickerForMaterial] = useState<
    number | null
  >(null);
  ...
  const [moduleRenames, setModuleRenames] = useState<
  ...
  const handleSaveEditedModule = () => {
    ...
    if (trimmed !== oldName) {
      setModuleRenames((prev) => [
        ...
        {
          from: prev.find((r) => r.to === oldName)?.from || oldName,
          to: trimmed,
        },
      ]);
    }
  ...
  >({});
  const [uploadingCoverStates, setUploadingCoverStates] = useState<
    Record<number, boolean>
  >({});
    ...
    fetch('/api/enrollments?depth=2')
      ...
      .then((data) => {
        ...
        docs.forEach((d: any) => {
          if (
            d.student &&
            typeof d.student === 'object' &&
            d.course?.id === savedCourseId
          ) {
  ...
  const handleAddBatch = async () => {
    ...
    if (
      ...
      Swal.fire({
        ...
        text: 'Please fill in batch name, instructor, and dates.',
        background: '#ffffff',
        color: '#1a1a1a',
      });
      return;
  ...
  const handleUpdateBatchField = async (
    batchId: string,
    field: 'maxStudents' | 'reactivateDate' | 'status',
    value: any,
  ) => {
    try {
      const res = await fetch(`/api/admin/batches/${batchId}`, {
  ...
  const handleAddStudentToBatch = async (
    ...
    if (!batch.isAcceptingStudents) {
      setBatchActionMsg(
        `This batch is full (max ${batch.maxStudents} students).`,
      );
  ...
  const handleRemoveStudentFromBatch = async (
    ...
    const updatedIds = batch.students
      .map((s) => s._id)
  ...
  const availableStudentsForSelectedBatch = courseStudents.filter(
    (s) =>
  ...
  const [slugStatus, setSlugStatus] = useState<'idle' | 'valid' | 'invalid'>(
    'idle',
  );
  ...
  const handleAddStudyMaterial = () => {
    setStudyMaterials([
      ...studyMaterials,
      { title: '', url: '', materialType: 'pdf', coverImage: '' },
    ]);
  };

  const handleStudyMaterialChange = (
    index: number,
    field: 'title' | 'url' | 'materialType',
    val: string,
  ) => {
  ...
  const handleStudyMaterialCoverUpload = async (
    index: number,
    ...
    formData.append(
      'alt',
      `Cover for ${studyMaterials[index]?.title || 'Study Material'}`,
    );
  ...
  const handleStudyMaterialUpload = async (
    index: number,
    e: React.ChangeEvent<HTMLInputElement>,
  ) => {
    const file = e.target.files?.[0];
      ...
      <div className='grid grid-cols-1 lg:grid-cols-12 gap-6'>
        ...
        <div className='lg:col-span-8 space-y-6'>
          ...
          <div className='bg-white border border-slate-200 rounded-lg p-6 space-y-5'>
            <h2 className='text-xl font-bold text-slate-800 tracking-tight border-b border-slate-100 pb-3'>
              Course Parameters
            </h2>
            ...
            <div className='flex flex-col gap-2'>
              ...
              <div className='flex items-center gap-2'>
                <span className='text-slate-400 font-semibold select-none hidden sm:inline'>
                  {domainOrigin}/courses/
                </span>
                <input
                  type='text'
                  required
              ...
              <div className='flex items-center gap-1.5 mt-1 text-sm font-semibold'>
                {slugChecking ? (
                  ...
                  <span className='text-emerald-400 flex items-center gap-1'>
                    <FiCheck /> Unique slug registered. URL path suffix is
                    available!
                  </span>
                ) : slugStatus === 'invalid' ? (
                  <span className='text-rose-400 flex items-center gap-1'>
                    <FiX /> Slug is taken! This path will overwrite or conflict.
                  </span>
                ) : null}
              </div>
            </div>
          ...
          <div className='bg-white border border-slate-200 rounded-lg p-6 space-y-4'>
            ...
            {requirements.length === 0 ? (
              <p className='text-base font-semibold text-slate-400 py-2'>
                No prerequisites defined yet. Specify baseline developer
                competencies.
              </p>
            ) : (
              <div className='space-y-2.5'>
            ...
            <div className='flex items-center justify-between border-b border-slate-100 pb-4'>
              <div>
                ...
                <p className='text-sm font-semibold text-slate-500 mt-1'>
                  Upload eBooks/PDFs or add external learning links
                </p>
              </div>
              <button
                type='button'
            ...
            {studyMaterials.length === 0 ? (
              <div className='text-center py-8 bg-slate-100 rounded-lg border border-dashed border-slate-200 p-6'>
                <p className='text-base font-semibold text-slate-500'>
                  No study materials added yet. Click &ldquo;Add Material&rdquo;
                  to get started.
                </p>
              </div>
            ) : (
              <div className='space-y-6'>
                {studyMaterials.map((item, idx) => (
                  <div
                    key={idx}
                    className='bg-slate-100 p-5 rounded-lg border border-slate-200/80 space-y-4 relative group'
                  >
                    <div className='flex justify-between items-center border-b border-slate-100 pb-2'>
                      <span className='text-base font-bold text-[#E61C24]'>
                        Material #{idx + 1}
                      </span>
                      <button
                        type='button'
                    ...
                    <div className='flex flex-col gap-2'>
                      ...
                      <div className='flex items-center gap-4'>
                        ...
                        <div className='flex flex-col gap-2'>
                          <div className='flex items-center gap-2'>
                            <label
                              className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-[#E61C24]/10 hover:bg-[#E61C24]/20 border border-[#E61C24]/20 text-[#E61C24] font-bold text-sm cursor-pointer transition-all ${uploadingCoverStates[idx] ? 'opacity-50 pointer-events-none' : ''}`}
                            >
                              <input
                                type='file'
                                accept='image/*'
                                onChange={(e) =>
                                  handleStudyMaterialCoverUpload(idx, e)
                                }
                                className='hidden'
                              />
                          ...
                          <p className='text-xs font-semibold text-slate-400'>
                            JPG, PNG, WEBP — 4:3 ratio recommended
                          </p>
                        </div>
                      </div>
                    </div>
          ...
          <div className='bg-white border border-slate-200 rounded-lg p-6 space-y-4'>
            ...
            <p className='text-sm font-semibold text-slate-400'>
              Create the curriculum modules for this course first. Later, you
              can select these modules when assigning lessons.
            </p>

            {modules.length === 0 ? (
              <p className='text-base font-semibold text-slate-400 py-2'>
                No modules defined yet. Course lessons will default to
                &ldquo;General Module&rdquo; if no modules are created.
              </p>
            ) : (
              <div className='space-y-2.5 pt-2'>
                {modules.map((modName, idx) => (
                  <div
                    key={idx}
                    className='flex items-center justify-between bg-slate-100 border border-slate-200 hover:border-zinc-750 px-4 py-3 rounded-lg transition-colors'
                  >
                    {editingModuleIndex === idx ? (
                      ...
                      <>
                        <div className='flex items-center gap-3 min-w-0'>
                          ...
                          <span className='text-slate-800 font-bold text-base truncate'>
                            {modName}
                          </span>
                        </div>
            ...
            {!savedCourseId ? (
              <div className='flex flex-col items-center justify-center py-10 gap-3 text-center'>
                ...
                <p className='text-base font-semibold text-slate-400'>
                  Add a title and modules above, then click &ldquo;Add
                  Lesson&quot; — the course will be saved as a draft
                  automatically.
                </p>
              </div>
            ) : lessonsLoading ? (
              ...
              <div className='space-y-2'>
                ...
                <p className='text-sm font-semibold text-slate-400 text-right pt-1'>
                  ...
                  {lessons.reduce((a, l) => a + l.duration, 0)} min total
                </p>
              </div>
            )}
          </div>
          ...
          <div className='bg-white border border-slate-200 rounded-lg p-6 space-y-4'>
            <div className='flex items-center justify-between border-b border-slate-100 pb-3'>
              <div>
                ...
                <p className='text-sm font-semibold text-slate-400 mt-1'>
                  Group enrolled students into intakes with optional seat
                  limits.
                </p>
              </div>
              {savedCourseId && (
            ...
            {!savedCourseId ? (
              <div className='flex flex-col items-center justify-center py-10 gap-3 text-center'>
                ...
                <p className='text-base font-semibold text-slate-400'>
                  Save the course first (or click &ldquo;Add Lesson&rdquo;
                  above) to start creating batches.
                </p>
              </div>
            ) : (
              <>
                    ...
                    <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                      ...
                      <div className='flex flex-col gap-1.5'>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setNewBatchInstructor(e.target.value)
                          }
                      ...
                      <div className='flex flex-col gap-1.5'>
                        <label className='text-base font-bold text-slate-600'>
                          Reactivate Date (Optional)
                        </label>
                        <input
                          type='date'
                          value={newBatchReactivate}
                          onChange={(e) =>
                            setNewBatchReactivate(e.target.value)
                          }
                          className='bg-white border border-slate-200 rounded-lg p-3 text-base font-semibold outline-none w-full cursor-pointer'
                        />
                        <p className='text-xs font-semibold text-slate-400'>
                          Once the seat limit is reached, joins reopen
                ...
                {batchesLoading ? (
                  ...
                  <p className='text-base font-semibold text-slate-400 py-2'>
                    No batches yet. Click &ldquo;Add Batch&rdquo; to create your
                    first intake group.
                  </p>
                ) : (
                  <div className='space-y-2.5'>
                    {batches.map((batch) => {
                      ...
                      return (
                        <div
                          key={batch._id}
                          className={`border rounded-lg transition-colors ${isSelected ? 'border-[#E61C24]/40 bg-[#E61C24]/5' : 'border-slate-200 bg-slate-50'}`}
                        >
                              ...
                              <div className='grid grid-cols-1 sm:grid-cols-3 gap-4'>
                                ...
                                <div className='flex flex-col gap-1.5'>
                                  ...
                                  <input
                                    type='date'
                                    defaultValue={
                                      batch.reactivateDate
                                        ? batch.reactivateDate.split('T')[0]
                                        : ''
                                    }
                                    onBlur={(e) =>
                                      handleUpdateBatchField(
                              ...
                              <div className='grid grid-cols-1 lg:grid-cols-2 gap-6 items-start'>
                                  ...
                                  <div className='border border-slate-200 rounded-lg bg-white p-3 max-h-[260px] overflow-y-auto space-y-2'>
                                    {batch.students.length === 0 ? (
                                      ...
                                      batch.students.map((s) => (
                                        <div
                                          key={s._id}
                                          className='p-3 rounded-lg bg-slate-50 border border-slate-100 flex items-center justify-between gap-3'
                                        >
                                ...
                                <div className='space-y-3'>
                                  ...
                                  <div className='border border-slate-200 rounded-lg bg-white p-3 max-h-[220px] overflow-y-auto space-y-2'>
                                    {availableStudentsForSelectedBatch.length ===
                                    0 ? (
                                      <p className='text-sm font-semibold text-slate-400 text-center py-6'>
                                        No matching enrolled students.
                                      </p>
                                    ) : (
                                      availableStudentsForSelectedBatch.map(
                                        (s) => (
                                          <div
                                            key={s._id}
                                            className='p-3 rounded-lg bg-slate-50 border border-slate-100 flex items-center justify-between gap-3'
                                  ...
                                  {!batch.isAcceptingStudents && (
                                    <p className='text-xs font-semibold text-rose-500'>
                                      This batch is full. Set a reactivate date
                                      or raise the max students limit to allow
                                      more joins.
                                    </p>
                                  )}
                                </div>
        ...
        <div className='lg:col-span-4 space-y-6'>
          {/* Thumbnail media box */}
          <div className='bg-white border border-slate-200 rounded-lg p-6 space-y-4'>
            <h3 className='text-xl font-bold text-slate-800 tracking-tight border-b border-slate-100 pb-2.5'>
            ...
            {thumbnailUrl ? (
              <div className='space-y-3'>
                ...
                <p className='text-xs font-semibold text-slate-400'>
                  <span className='font-bold text-slate-500'>Alt:</span> &ldquo;
                  {thumbnailAlt}&rdquo;
                </p>
                {/* Change buttons */}
                ...
                {uploadingImage ? (
                  ...
                  <>
                    ...
                    >
                      ...
                      <p className='text-sm font-semibold text-slate-400'>
                        PNG, JPEG, WEBP — Recommended size: 1280x720 (16:9
                        ratio)
                      </p>
                    </button>
          ...
          {showMediaPickerForMaterial !== null && (
            <MediaPickerModal
              ...
              onSelect={(media: MediaItem) => {
                const updated = [...studyMaterials];
                updated[showMediaPickerForMaterial] = {
                  ...updated[showMediaPickerForMaterial],
            ...
            <div className='flex flex-col gap-2'>
              ...
              >
                ...
                <option value='published'>
                  Published (Render on Public Landing Page)
                </option>
              </select>
            </div>
          </div>
                ...
                >
                  ...
                  {categoriesOptions.map((cat) => (
                    <option
                      key={cat.id}
                      value={cat.id}
                      disabled={categories.includes(cat.id)}
                    >
                      {cat.name}
                    </option>
                  ))}
                ...
                <div className='flex flex-wrap gap-2 mt-2'>
                  {categories.map((catId) => {
                    ...
                    return (
                      <span
                        key={catId}
                        className='bg-slate-100 border border-slate-200 text-slate-700 px-3 py-1.5 rounded-full text-sm font-semibold flex items-center gap-2'
                      >
                        {catName}
                        <button
                          type='button'
                          onClick={() => handleRemoveCategory(catId)}
                          className='text-slate-400 hover:text-[#E61C24] transition-colors'
                        >
            ...
            <div className='flex flex-col gap-2'>
              ...
              {user.role === 'instructor' ? (
                <div className='bg-slate-100 border border-slate-200 text-slate-500 rounded-lg p-3 text-base font-semibold select-none flex items-center gap-2'>
                  ...
                  <span>
                    {instructorsOptions.find((ins) => ins.id === user.id)
                      ?.name || 'You (Assigned Mentor)'}
                  </span>
                </div>
              ) : (
                <>
                  <div className='flex gap-2'>
                    ...
                    >
                      ...
                      <option
                        value={user.id}
                        disabled={instructors.includes(user.id)}
                      >
                        Myself ({user.role === 'admin' ? 'Admin' : 'Staff'})
                      </option>
                      {instructorsOptions.map((ins) => (
                        <option
                          key={ins.id}
                          value={ins.id}
                          disabled={instructors.includes(ins.id)}
                        >
                          {ins.name} ({ins.email})
                        </option>
                      ))}
                  ...
                  {instructors.length > 0 && (
                    <div className='flex flex-wrap gap-2 mt-2'>
                      {instructors.map((insId) => {
                        const insData = instructorsOptions.find(
                          (i) => i.id === insId,
                        );
                        const insName =
                          insId === user.id
                            ? `Myself (${user.role === 'admin' ? 'Admin' : 'Staff'})`
                            : insData?.name || 'Unknown';
                        return (
                          <span
                            key={insId}
                            className='bg-slate-100 border border-slate-200 text-slate-700 px-3 py-1.5 rounded-full text-sm font-semibold flex items-center gap-2'
                          >
                            ...
                            <button
                              type='button'
                              onClick={() => handleRemoveInstructor(insId)}
                              className='text-slate-400 hover:text-[#E61C24] transition-colors'
                            >
                              <FiX className='h-4 w-4' />
                            </button>
