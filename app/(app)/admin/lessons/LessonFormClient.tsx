...
export default function LessonFormClient({
  ...
  const defaultCourseId =
    initialData?.courseId || searchParams.get('courseId') || '';
  ...
  const [moduleName, setModuleName] = useState(
    initialData?.moduleName || 'General Module',
  );
  ...
  const [moduleOrder, setModuleOrder] = useState<number | ''>(
    initialData?.moduleOrder || '',
  );
  const [content, setContent] = useState<string>(initialData?.content || '');
  const [lessonType, setLessonType] = useState<
    'recorded' | 'live' | 'quiz' | 'assignment'
  ...
  const [isPreviewable, setIsPreviewable] = useState(
    initialData?.isPreviewable || false,
  );
  const [autoGenerateZoom, setAutoGenerateZoom] = useState(
    initialData?.autoGenerateZoom || false,
  );
  ...
  const [showMediaPickerForQuestion, setShowMediaPickerForQuestion] = useState<
    number | null
  >(null);
  ...
  React.useEffect(() => {
    ...
    fetch(`/api/admin/lessons?courseId=${courseId}`)
      ...
      .then((data) => {
        ...
        const maxCourseOrder = lessons.reduce(
          (max: number, l: any) => Math.max(max, l.order || 0),
          0,
        );
        setOrder(maxCourseOrder + 1);
        const inModule = lessons.filter(
          (l) =>
            (l.moduleName || 'General Module') ===
            (moduleName || 'General Module'),
        );
        const maxModuleOrder = inModule.reduce(
          (max: number, l: any) => Math.max(max, l.moduleOrder || 0),
          0,
        );
        setModuleOrder(
          maxModuleOrder > 0 ? maxModuleOrder + 1 : inModule.length + 1,
        );
      })
      .catch(() => {})
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    if (lessonType === 'quiz') {
      const invalid = quizQuestions.some(
    ...
    try {
      const payload = {
        ...
        isPreviewable,
        autoGenerateZoom:
          lessonType === 'live' && livePlatform === 'zoom'
            ? autoGenerateZoom
            : false,
      ...
      >
        ...
        <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-4 border-b border-slate-200/40'>
          ...
          <button
            ...
            onClick={() =>
              router.push(
                courseId ? `/admin/courses/${courseId}/edit` : '/admin/lessons',
              )
            }
            ...
            <div className='bg-white border border-slate-200 rounded-lg p-6 space-y-5'>
              ...
              <div className='flex flex-col gap-2'>
                ...
                <p className='text-base font-medium text-slate-400 leading-relaxed'>
                  Select from the curriculum modules defined for this course.
                  (Add/edit modules in the Course parameters page).
                </p>
              </div>
              ...
              <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
                ...
                <div className='flex flex-col gap-2'>
                  ...
                  <p className='text-sm font-semibold text-slate-400'>
                    What number this lesson is inside its module (1st lesson of
                    a module = 1). Auto-counted; change if needed.
                  </p>
                </div>
              </div>
              ...
              <div className='flex flex-col gap-2 pt-2'>
                ...
                <div className='flex flex-wrap gap-3'>
                  {(['recorded', 'live', 'quiz', 'assignment'] as const).map(
                    (type) => (
                      <button
                        key={type}
                        type='button'
              ...
              {lessonType === 'recorded' ? (
                <div className='flex flex-col gap-4 animate-fadeIn'>
                  <div className='space-y-2'>
                    <label className='text-base font-bold text-slate-600'>
                      📹 Upload Video to R2 (Recommended)
                    </label>
                    <VideoUploadWidget
                  ...
                  <div className='space-y-2'>
                    ...
                    <p className='text-base font-medium text-slate-400 leading-relaxed'>
                      ...
                      (e.g.{' '}
                      <span className='font-mono text-[#FF4D55]'>
                        videos/lesson-1.mp4
                      </span>
                      ).
                    </p>
                ...
                <div className='space-y-4 animate-fadeIn'>
                  <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
                    ...
                    <div className='flex flex-col gap-2'>
                      <label className='text-base font-bold text-slate-600'>
                        Scheduled Time & Date (Bangladesh time)
                      </label>
                      <input
                        type='datetime-local'
                        value={liveDate}
                  ...
                  {livePlatform === 'zoom' && (
                    <div className='bg-slate-100 border border-slate-200/80 rounded-lg p-4 flex flex-col gap-3 animate-fadeIn'>
                      ...
                      <p className='text-base font-medium text-slate-500'>
                        When enabled, Canadian Nest School will automatically
                        create a Zoom meeting using your Server-to-Server OAuth
                        credentials.
                      </p>
                    </div>
                  )}
                  ...
                  <div className='flex flex-col gap-2'>
                    <label className='text-base font-bold text-slate-600'>
                      {autoGenerateZoom
                        ? 'Meeting Join URL (Auto-generated)'
                        : 'Meeting Join URL'}
                    </label>
                    <input
                      type='url'
                      ...
                      placeholder={
                        autoGenerateZoom
                          ? 'Will be automatically generated upon save'
                          : 'https://zoom.us/j/...'
                      }
                      className={`bg-slate-100 border border-slate-200 focus:border-[#E61C24]/80 focus:ring-1 focus:ring-[#E61C24]/80 text-slate-800 rounded-lg p-3 text-base font-semibold outline-none w-full transition-colors font-mono ${
                        autoGenerateZoom
                ...
                <div className='space-y-4 border border-[#E61C24]/25 rounded-lg p-6 bg-linear-to-b from-slate-50 to-white shadow-2xl shadow-[#E61C24]/5 animate-fadeIn'>
                  <h3 className='text-xl font-bold text-slate-800 flex items-center gap-2 border-b border-slate-100 pb-3'>
                    <FiFileText className='text-[#E61C24] h-6 w-6' /> Assignment
                    Configuration
                  </h3>

                  <div className='flex flex-col gap-2'>
                    <label className='text-base font-bold text-slate-600'>
                      Assignment Evaluation Marks *
                    </label>
                    <input
                      type='number'
                      min={1}
                  ...
                  <p className='text-base font-semibold text-slate-500'>
                    Students will see this assignment and submit a secure Google
                    Drive link containing their work for grading.
                  </p>
                </div>
              ) : (
                <div className='space-y-6 border border-[#E61C24]/25 rounded-lg p-6 bg-linear-to-b from-slate-50 to-white shadow-2xl shadow-[#E61C24]/5 animate-fadeIn'>
                  <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200/60 pb-4'>
                    <div>
                      <h3 className='text-xl font-bold text-slate-800 flex items-center gap-2'>
                        <FiHelpCircle className='text-[#E61C24] h-6 w-6' /> Quiz
                        Questions Builder
                      </h3>
                      <p className='text-base font-medium text-slate-500 mt-1'>
                        Configure dynamic evaluation queries with dynamic option
                        selection
                      </p>
                    </div>
                    <div className='flex flex-col sm:flex-row sm:items-center gap-3'>
                      ...
                      <button
                        ...
                        className='flex items-center gap-2 px-5 py-2.5 bg-linear-to-r from-[#E61C24] to-[#CC181F] hover:from-[#CC181F] hover:to-[#4338ca] text-slate-800 rounded-lg text-base font-bold shadow-lg shadow-[#E61C24]/20 transition-all hover:scale-[1.02] active:scale-[0.98] cursor-pointer border-none'
                      >
                      ...
                      >
                        ...
                        <div className='flex items-center justify-between pb-4 border-b border-slate-100/50'>
                          <div className='flex items-center gap-3'>
                            <span className='flex h-9 w-9 items-center justify-center rounded-lg bg-linear-to-br from-[#E61C24] to-[#CC181F] text-white font-bold text-base shadow-md shadow-[#E61C24]/20 select-none'>
                            ...
                            <div>
                              ...
                              <span className='text-base font-medium text-slate-500 block mt-0.5'>
                                Define your question and choices below
                              </span>
                            </div>
                          </div>
                        ...
                        <div className='flex flex-col gap-2'>
                          ...
                          {q.imageUrl ? (
                            ...
                            <button
                              type='button'
                              onClick={() =>
                                setShowMediaPickerForQuestion(qIdx)
                              }
                        ...
                        <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pt-2'>
                          <div>
                            ...
                            <p className='text-base font-medium text-slate-500 mt-0.5'>
                              Add up to 6 options and mark the correct one.
                            </p>
                          </div>
                          {q.options.length < 6 && (
                            <button
                            ...
                            return (
                              ...
                              >
                                      ...
                                      <button
                                        ...
                                        onClick={() => {
                                          ...
                                          newQuestions[qIdx].options =
                                            newQuestions[qIdx].options.filter(
                                              (_, idx) => idx !== optIdx,
                                            );
                                ...
                                <input
                                  ...
                                  onChange={(e) => {
                                    const newQuestions = [...quizQuestions];
                                    newQuestions[qIdx].options[optIdx] =
                                      e.target.value;
                                    setQuizQuestions(newQuestions);
                                  }}
              ...
              <div className='flex flex-col gap-2'>
                ...
                <p className='text-xs text-slate-500 font-semibold mt-1'>
                  This lesson&apos;s serial number across the whole course.
              ...
              <div className='flex flex-col gap-2 pt-4 border-t border-slate-100 mt-4'>
                ...
                <p className='text-sm font-medium text-slate-500 mb-2'>
                  Optional content shown beneath the lesson video or assignment.
                </p>
                <RichTextEditor
                  value={content}
                  onChange={setContent}
