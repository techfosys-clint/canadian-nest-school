...
export default function ProductFormClient({ initialData }: ProductFormProps) {
  ...
  const [productType, setProductType] = useState<
    'book' | 'merchandise' | 'other'
  >(initialData?.productType || 'book');
  ...
  const handleGalleryUpload = async (
    ...
    try {
      ...
      Swal.fire({
        icon: 'error',
        title: 'Upload Failed',
        text: err.message || 'Could not upload gallery image.',
        background: '#ffffff',
        color: '#1a1a1a',
      });
    } finally {
          ...
          <div className='bg-white border border-slate-200 rounded-lg p-6 space-y-4'>
            <div className='flex items-center justify-between border-b border-slate-100 pb-3'>
              ...
              <div className='flex items-center gap-2'>
                <label
                  className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-[#E61C24]/10 hover:bg-[#E61C24]/20 border border-[#E61C24]/20 text-[#E61C24] font-bold text-sm cursor-pointer transition-all ${uploadingGallery ? 'opacity-50 pointer-events-none' : ''}`}
                >
                  <input
                    ref={galleryFileRef}
                    type='file'
        ...
        <div className='space-y-6'>
          ...
          <div className='bg-white border border-slate-200 rounded-lg p-6 space-y-4'>
            ...
            <div className='flex gap-2'>
              <label
                className={`flex-1 inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-[#E61C24]/10 hover:bg-[#E61C24]/20 border border-[#E61C24]/20 text-[#E61C24] font-bold text-sm cursor-pointer transition-all ${uploadingThumbnail ? 'opacity-50 pointer-events-none' : ''}`}
              >
                <input
