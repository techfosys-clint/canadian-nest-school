...
import { FiPlus, FiEdit, FiTrash2, FiShoppingBag, FiImage, FiBook } from 'react-icons/fi'
  ...
  return (
    <div className="px-6 py-8 space-y-6 container mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200 pb-6">
        <div>
          <h1 className="text-3xl font-bold font-display text-slate-800">Shop Products</h1>
      ...
      <div className="bg-white border border-slate-200 rounded-lg overflow-hidden shadow-sm">
        {products.length === 0 ? (
          <div className="p-16 text-center space-y-4">
            ...
            <h3 className="text-lg font-bold text-slate-600">No products yet</h3>
              ...
              <tbody className="divide-y divide-slate-100">
                {products.map((product) => (
                  <tr key={product.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-sm font-semibold text-slate-400 truncate">/{product.slug}</p>
                        </div>
                      </div>
                    </td>
                    ...
                    {/* Price */}
                    <td className="px-6 py-4">
                      <span className="font-bold text-slate-800">৳{product.price.toLocaleString('en-BD')}</span>
                    ...
                    <td className="px-6 py-4">
                      <span className="font-semibold text-slate-600">
                        {product.stock === null || product.stock === undefined ? 'Unlimited' : product.stock}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      ...
                      >
                        <span className={`h-1.5 w-1.5 rounded-full ${product.status === 'published' ? 'bg-emerald-500 animate-pulse' : 'bg-slate-400'}`} />
                        <span>{product.status === 'published' ? 'Published' : 'Draft'}</span>
                      </button>
                    </td>
