...
export default function StaffEditFormClient({
  initialStaff,
}: StaffEditFormClientProps) {
  ...
  const [designation, setDesignation] = useState(
    initialStaff.designation || '',
  );
  const [password, setPassword] = useState('');
  const [role, setRole] = useState<'staff' | 'instructor' | 'admin'>(
    initialStaff.role,
  );
  const [permissions, setPermissions] = useState<string[]>(
    initialStaff.permissions || [],
  );
  ...
  const [profilePicId, setProfilePicId] = useState(
    initialStaff.profilePicId || '',
  );
  const [profilePicUrl, setProfilePicUrl] = useState(
    initialStaff.profilePicUrl || '',
  );
    ...
    <div className='w-full bg-white border border-slate-200/80 rounded-lg p-6 md:p-8 shadow-sm space-y-8 select-text'>
        ...
        <div className='flex items-center gap-4'>
          ...
          <div>
            ...
            <p className='text-sm font-medium text-slate-500 mt-2'>
              Modify information for administrative or teacher account
            </p>
          </div>
        </div>
        <button
      ...
      <form onSubmit={handleSubmit} className='space-y-8 text-base font-sans'>
        ...
        <div className='space-y-6'>
          ...
          <div className='flex flex-col gap-2'>
            <label className='text-sm font-bold text-slate-700'>
              Staff Job Designation / Professional Role{' '}
              <span className='text-slate-400 font-normal'>(Optional)</span>
            </label>
            <div className='relative'>
              <FiUser className='absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 h-5 w-5' />
            ...
            <p className='text-xs text-slate-500 font-medium'>
              This designation will display on public views (like the Mentors
              page).
            </p>
          </div>
          ...
          <div className='flex flex-col gap-3'>
            ...
            <div className='flex items-center gap-5 flex-wrap bg-slate-50 border border-slate-200 p-5 rounded-lg shadow-sm'>
              ...
              <div className='flex items-center gap-3'>
                <label
                  className={`flex items-center gap-2 px-4 py-2 rounded-lg bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 font-semibold text-sm cursor-pointer transition-colors shadow-sm ${uploading ? 'opacity-50 pointer-events-none' : ''}`}
                >
                  <input
                    type='file'
                    accept='image/*'
          ...
          <div className='flex flex-col gap-3 pt-4 border-t border-slate-100'>
            <div>
              ...
              <p className='text-xs text-slate-500 font-medium mt-1'>
                Select the primary access level for this account.
              </p>
            </div>
            <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
              ...
              >
                ...
                />
                <FiShield
                  className={`h-5 w-5 shrink-0 mt-0.5 ${role === 'staff' ? 'text-rose-500' : 'text-slate-400'}`}
                />
                <div className='text-left'>
                  <p
                    className={`font-bold text-base leading-none ${role === 'staff' ? 'text-slate-800' : 'text-slate-700'}`}
                  >
                    Console Staff
                  </p>
                  <p className='text-xs font-medium text-slate-500 mt-2 leading-relaxed'>
                    Manage FAQs, Blogs, Categories
                  </p>
                </div>
              </label>
              ...
              >
                ...
                />
                <FiShield
                  className={`h-5 w-5 shrink-0 mt-0.5 ${role === 'instructor' ? 'text-rose-500' : 'text-slate-400'}`}
                />
                <div className='text-left'>
                  <p
                    className={`font-bold text-base leading-none ${role === 'instructor' ? 'text-slate-800' : 'text-slate-700'}`}
                  >
                    Instructor
                  </p>
                  <p className='text-xs font-medium text-slate-500 mt-2 leading-relaxed'>
                    Manage Syllabus, Create Lessons
                  </p>
                </div>
              </label>
              ...
              >
                ...
                />
                <FiShield
                  className={`h-5 w-5 shrink-0 mt-0.5 ${role === 'admin' ? 'text-rose-500' : 'text-slate-400'}`}
                />
                <div className='text-left'>
                  <p
                    className={`font-bold text-base leading-none ${role === 'admin' ? 'text-slate-800' : 'text-slate-700'}`}
                  >
                    Operations
                  </p>
                  <p className='text-xs font-medium text-slate-500 mt-2 leading-relaxed'>
                    Full system access, Add other staff
                  </p>
                </div>
              </label>
            </div>
            ...
            <div className='flex flex-col gap-4 pt-6 border-t border-slate-100'>
              <div>
                ...
                <p className='text-xs font-medium text-slate-500 mt-1'>
                  Configure exactly which pages this administrative account can
                  read and write
                </p>
              </div>
