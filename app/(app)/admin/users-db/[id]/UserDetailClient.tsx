...
export default function UserDetailClient({ id }: { id: string }) {
  ...
  const handleToggleStatus = async () => {
    ...
    try {
      ...
      Swal.fire(
        'Updated!',
        `Account has been ${newStatus === 'suspended' ? 'blocked' : 'unblocked'}.`,
        'success',
      );
    } catch (err: any) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(
        `/api/admin/users-db?id=${user.id}&type=${user.type}`,
        { method: 'DELETE' },
      );
      const data = await res.json();
  ...
  if (error || !user) {
    return (
      <div className='p-8 max-w-3xl mx-auto'>
        <Link
          href='/admin/users-db'
          className='inline-flex items-center gap-2 text-sm font-bold text-slate-500 hover:text-slate-800 mb-6'
        >
  ...
  const totalSpent = completedEnrollments.reduce(
          ...
          <div className='flex-1 min-w-0 space-y-2'>
            ...
            <div className='flex flex-col sm:flex-row sm:flex-wrap gap-x-6 gap-y-1.5 text-sm font-semibold text-slate-600'>
              <span className='inline-flex items-center gap-2'>
                <FiMail className='h-4 w-4 text-[#E61C24]' />{' '}
                {user.email || '—'}
              </span>
              <span className='inline-flex items-center gap-2'>
                <FiPhone className='h-4 w-4 text-[#E61C24]' />{' '}
                {user.phone || '—'}
              </span>
              <span className='inline-flex items-center gap-2'>
                <FiCalendar className='h-4 w-4 text-[#E61C24]' /> Joined{' '}
          ...
          <div className='bg-white border border-slate-200 rounded-lg shadow-sm p-6'>
            ...
            <div className='flex flex-wrap gap-2'>
              {user.permissions.map((perm) => (
                <span
                  key={perm}
                  className='px-3 py-1 rounded-lg bg-slate-100 border border-slate-200 text-sm font-semibold text-slate-600 capitalize'
                >
                  {perm}
                </span>
              ))}
        ...
        <>
          ...
          <div className='grid grid-cols-2 sm:grid-cols-3 gap-4'>
            ...
            <div className='bg-white border border-slate-200 rounded-lg p-5'>
              ...
              <p className='text-3xl font-bold text-[#E61C24] mt-1'>
                  ...
                  >
                    <div className='min-w-0'>
                      <div className='flex items-center gap-2'>
                        {enr.courseSlug ? (
                          <Link
                            href={`/courses/${enr.courseSlug}`}
                            className='font-bold text-slate-800 hover:text-[#E61C24] transition-colors truncate'
                          >
                            {enr.courseTitle}
                          </Link>
                        ) : (
                        ...
                        <span
                          className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-bold uppercase tracking-wide border ${PAYMENT_STYLES[enr.paymentStatus] || PAYMENT_STYLES.refunded}`}
                        >
                      ...
                      <div className='flex flex-wrap items-center gap-x-4 gap-y-1 text-sm font-semibold text-slate-450 mt-1'>
                        ...
                        {enr.paymentReference && (
                          <span className='font-mono text-slate-500'>
                            Ref: {enr.paymentReference}
                          </span>
                        )}
                        {enr.couponCode && (
                          <span className='inline-flex items-center gap-1 text-emerald-600'>
                            <FiTag className='h-3.5 w-3.5' /> {enr.couponCode}
                          </span>
                        )}
                      </div>
                    </div>
                    <span className='text-lg font-bold text-slate-800 shrink-0'>
                      ৳{(enr.pricePaid || 0).toLocaleString('en-BD')}
                    </span>
                  </div>
                ))}
              </div>
