...
const statusConfig: Record<string, { label: string; classes: string }> = {
  pending: {
    ...
    classes:
      'bg-amber-500/10 text-amber-600 rounded-lg text-sm px-2.5 py-0.5 font-bold',
  },
  approved: {
    label: 'Live & Published',
    classes:
      'bg-emerald-500/10 text-emerald-600 rounded-lg text-sm px-2.5 py-0.5 font-bold',
  },
  rejected: {
    label: 'Not Approved',
    classes:
      'bg-rose-500/10 text-rose-600 rounded-lg text-sm px-2.5 py-0.5 font-bold',
  },
};
    ...
    async function init() {
      try {
        ...
        const reviewRes = await fetch(
          `/api/reviews?where[student][equals]=${encodeURIComponent(sessionData.user.id)}&depth=2&limit=50`,
        );
        if (reviewRes.ok) {
      ...
      <div className='w-full bg-[#0A163A] rounded-lg p-8 md:p-12 relative overflow-hidden mb-10 border border-zinc-800/20'>
        ...
        <div className='relative z-10 max-w-2xl'>
          ...
          <p className='text-zinc-400 text-base md:text-lg font-semibold leading-relaxed'>
            Your dynamic course feedback helps shape curriculum quality. Track
            your logged evaluations and add new course testimonials!
          </p>
        </div>
      </div>
        ...
        <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4'>
          <div>
            ...
            <p className='text-base font-semibold text-zinc-450 mt-1'>
              Timeline of submitted course course logs
            </p>
          </div>
            ...
            <AnimatePresence>
              {myReviews.length === 0 ? (
                ...
                myReviews.map((review) => {
                  const courseTitle =
                    review.course && typeof review.course === 'object'
                      ? review.course.title
                      : 'Course';
                  const cfg =
                      ...
                      {review.status === 'approved' && (
                        <div className='flex items-center gap-1.5 text-emerald-600'>
                          ...
                          <span className='text-sm font-bold'>
                            Published on course landing page
                          </span>
                        </div>
                      )}
                    </motion.div>
