...
export default function ForgotPasswordForm() {
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!email) {
      Swal.fire({ icon: 'error', title: 'Missing Email', text: 'Please enter your email address.', ...swalTheme })
      return
    }
    ...
    try {
      ...
      Swal.fire({ icon: 'error', title: 'Reset Failed', text: err.message || 'Something went wrong.', ...swalTheme })
    } finally {
  ...
  const handleSendPhoneOtp = async () => {
    if (!phone) {
      Swal.fire({ icon: 'error', title: 'Missing Phone Number', text: 'Please enter your mobile number.', ...swalTheme })
      return
    }
    ...
    try {
      ...
      Swal.fire({ icon: 'error', title: 'Failed to Send OTP', text: err.message || 'Something went wrong.', ...swalTheme })
    } finally {
      setSendingOtp(false)
    }
  }
  ...
  const handlePhoneResetSubmit = async (e: React.FormEvent) => {
    ...
    if (!otp) {
      Swal.fire({ icon: 'error', title: 'Missing OTP', text: 'Please enter the verification code sent to your phone.', ...swalTheme })
      return
    }
    if (newPassword.length < 6) {
      Swal.fire({ icon: 'error', title: 'Weak Password', text: 'Password must be at least 6 characters long.', ...swalTheme })
      return
    }
    if (newPassword !== confirmPassword) {
      Swal.fire({ icon: 'error', title: 'Passwords Do Not Match', text: 'Please make sure both password fields match.', ...swalTheme })
      return
    }
    ...
    try {
      ...
      if (res.ok) {
        await Swal.fire({ icon: 'success', title: 'Password Reset!', text: 'Your password has been reset successfully.', ...swalTheme })
      ...
      <aside className="hidden md:flex md:w-1/2 flex-col justify-between bg-[#070b19] p-12 text-white relative select-none">
        ...
        <div className="relative z-10">
          <Link href="/" className="inline-flex items-center group bg-white px-3 py-1.5 rounded-lg shadow-sm w-fit">
            <img src="/logo.png" alt="Canadian Nest School" className="h-12 w-auto object-contain" />
          </Link>
        </div>
        ...
        <div className="w-full max-w-md space-y-8">
          ...
          <div className="flex items-center justify-between md:hidden mb-6">
            <Link href="/" className="flex items-center group">
              <img src="/logo.png" alt="Canadian Nest School" className="h-10 w-auto object-contain" />
            </Link>
            ...
            <form onSubmit={handlePhoneResetSubmit} className="space-y-5">
              ...
              {otpSent && (
                <>
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label htmlFor="newPassword" className="text-base font-bold text-zinc-700">
                      New Password
                    </label>
                    <div className="relative">
