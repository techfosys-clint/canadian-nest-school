      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
              ...
              {gallery.map((url, idx) => (
                <button
                  ...
                  className={`aspect-square rounded-lg overflow-hidden border-2 transition-all cursor-pointer ${
                    activeImage === url ? 'border-[#E61C24]' : 'border-transparent hover:border-zinc-200'
                  }`}
                >
        ...
        <div className="lg:col-span-7 space-y-6">
          <div className="space-y-2">
            ...
            <h1 className="text-3xl font-bold text-zinc-900 leading-tight">{product.title}</h1>
            {product.author && (
              <p className="text-base font-semibold text-zinc-500">by {product.author}</p>
            )}
          </div>

          <div className="flex items-center gap-3">
            <span className="text-3xl font-bold text-[#E61C24]">{formatPrice(product.price)}</span>
          ...
          {product.shortDescription && (
            <p className="text-base font-semibold text-zinc-600 leading-relaxed">{product.shortDescription}</p>
          )}
          ...
          {product.description && (
            <div className="bg-white border border-zinc-200/80 rounded-lg p-6">
              <h2 className="text-xl font-bold text-zinc-800 mb-3 border-b border-zinc-100 pb-3">Description</h2>
              <div
