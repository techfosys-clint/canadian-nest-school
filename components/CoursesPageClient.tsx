  ...
  if (view === 'list') {
    return (
      ...
      >
        ...
        <div className='shrink-0 w-full md:w-60 aspect-16/10 rounded-lg overflow-hidden bg-zinc-50 relative'>
          {imgUrl ? (
            ...
            <div className='w-full h-full bg-linear-to-br from-[#0A163A] to-[#1e1b4b] flex items-center justify-center rounded-lg'>
              <span className='h-10 w-10 rounded-lg bg-[#E61C24]/30 flex items-center justify-center font-bold text-white text-base'>
                T
              </span>
            </div>
          )}
        ...
        {/* Info */}
        <div className='grow min-w-0 flex flex-col justify-between py-1 gap-4'>
          ...
          <div className='flex items-center justify-between gap-2 pt-1'>
            <div className='flex flex-wrap items-baseline gap-x-1.5 min-w-0'>
              <span
                className={`text-lg sm:text-xl font-bold ${course.price === 0 ? 'text-emerald-600' : 'text-zinc-900'} truncate`}
              >
  ...
  return (
    <div className='w-full flex justify-center sm:justify-start'>
      ...
      >
        ...
        <div className='relative aspect-16/10 bg-zinc-50 overflow-hidden rounded-t-lg'>
          {imgUrl ? (
            ...
            <div className='w-full h-full bg-linear-to-br from-[#0A163A] to-[#1e1b4b] flex flex-col items-center justify-center rounded-t-lg'>
              <span className='h-12 w-12 rounded-lg bg-[#E61C24]/25 flex items-center justify-center font-bold text-white mb-2 text-lg'>
                C
              </span>
              <span className='text-base font-bold text-zinc-400 uppercase tracking-wider'>
                Canadian Nest
              </span>
            </div>
          )}
        ...
        {/* Content Details */}
        <div className='p-5 md:p-6 grow flex flex-col justify-between gap-4'>
          <div className='space-y-2'>
          ...
          <div className='space-y-4'>
            ...
            <div className='flex items-center justify-between gap-2 pt-1'>
              <div className='flex flex-wrap items-baseline gap-x-1.5 min-w-0'>
                <span
                  className={`text-xl font-bold ${course.price === 0 ? 'text-emerald-600' : 'text-zinc-900'} truncate`}
                >
                  {formatPrice(course.price)}
                </span>
              </div>
  ...
  return (
    <div className='py-1'>
      ...
      >
        <span>{title}</span>
        <FiChevronDown
          className={`h-5 w-5 text-zinc-400 transition-transform duration-200 ${open ? '' : '-rotate-90'}`}
        />
      </button>
...
export default function CoursesPageClient({
  ...
  const filtered = useMemo(() => {
    ...
    if (activeCategory !== 'all') {
      result = result.filter(
        (c) => getCategorySlug(c.category) === activeCategory,
      );
    }
  ...
  return (
    <div className='min-h-screen bg-linear-to-b from-white via-[#fafafa] to-[#f9fafb] font-sans pb-20 relative flex flex-col pt-22'>
      ...
      <div className='container mx-auto px-6 py-8 flex flex-col gap-8 relative z-10 grow'>
        ...
        <div className='flex flex-col lg:flex-row gap-8 items-start relative mt-4'>
          ...
          <aside className='hidden lg:block w-72 shrink-0 space-y-6 lg:sticky lg:top-28 z-20'>
            ...
            <div className='bg-white rounded-lg p-6 shadow-sm hover:shadow-md transition-shadow space-y-4 border-none'>
              <h3 className='text-xl font-bold text-[#0A163A]'>
                Browse Categories
              </h3>
              ...
              <div className='space-y-1 mt-2'>
                ...
                >
                  ...
                  <span
                    className={`text-base font-bold rounded-lg px-2.5 py-0.5 ${activeCategory === 'all' ? 'bg-white/20 text-white' : 'bg-zinc-100 text-zinc-500'}`}
                  >
                    {courses.length}
                  </span>
                </button>
                ...
                {displayedCategories.map((cat) => {
                  const count = courses.filter(
                    (c) => getCategorySlug(c.category) === cat.slug,
                  ).length;
                  ...
                  return (
                    ...
                    >
                      <span className='flex items-center gap-2 truncate'>
                        <FiChevronRight
                          className={`h-4.5 w-4.5 transition-transform ${active ? 'text-[#E61C24] translate-x-0.5' : 'text-zinc-400'}`}
                        />
                        <span className='truncate'>{cat.name}</span>
                      </span>
                      <span
                        className={`text-base font-bold rounded-lg px-2.5 py-0.5 ${active ? 'bg-[#E61C24]/15 text-[#E61C24]' : 'bg-zinc-150 text-zinc-500'}`}
                      >
                        {count}
                      </span>
                    </button>
            ...
            <div className='bg-white rounded-lg p-6 shadow-sm hover:shadow-md transition-shadow space-y-6 border-none'>
              ...
              <div className='space-y-3'>
                <p className='text-base font-bold text-[#0A163A] uppercase tracking-wider'>
                  Tier
                </p>
                      ...
                      <span
                        className={`text-base font-semibold transition-colors duration-200 ${
                          priceTier === tier.value
                            ...
                            : 'text-zinc-650 group-hover:text-zinc-800'
                ...
                <div className='space-y-2.5'>
                  {LANGUAGES.map((lang) => (
                    ...
                    >
                      ...
                      <span
                        className={`text-base font-semibold transition-colors duration-200 ${
                          activeLanguages.includes(lang.value)
                            ? 'text-[#E61C24] font-bold'
                            : 'text-zinc-650 group-hover:text-zinc-800'
                        }`}
                      >
                ...
                <div className='space-y-2.5'>
                  {LEVELS.map((lvl) => (
                    ...
                    >
                      ...
                      <span
                        className={`text-base font-semibold transition-colors duration-200 ${
                          activeLevels.includes(lvl.value)
                            ? 'text-[#E61C24] font-bold'
                            : 'text-zinc-650 group-hover:text-zinc-800'
                        }`}
                      >
          ...
          <main className='grow min-w-0 w-full space-y-6'>
            ...
            <div className='flex flex-col sm:flex-row gap-4 items-center bg-white rounded-lg p-3 border border-zinc-200'>
              <div className='grow flex items-center gap-3 pl-3 w-full'>
              ...
              <div className='flex items-center gap-2.5 w-full sm:w-auto shrink-0 justify-between sm:justify-end'>
                ...
                <button
                  ...
                  className='lg:hidden flex items-center gap-2 px-4 py-2.5 bg-white border border-zinc-200 rounded-lg text-base font-bold text-[#0A163A] hover:bg-zinc-50 transition-all cursor-pointer grow sm:grow-0 justify-center'
                >
            ...
            <AnimatePresence mode='wait'>
              {filtered.length === 0 ? (
                ...
                >
                  ...
                  <p className='text-base text-zinc-450 max-w-xs mb-6 font-semibold'>
