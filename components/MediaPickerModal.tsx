...
export default function MediaPickerModal({
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        if (res.ok && data.media) {
          ...
          setUploads((prev) =>
            prev.map((u, idx) =>
              idx === i ? { ...u, status: 'done', url: item.url } : u,
            ),
          );
        } else {
          setUploads((prev) =>
            prev.map((u, idx) =>
              idx === i
                ? { ...u, status: 'error', error: data.error || 'Failed' }
                : u,
            ),
          );
        }
  ...
  return (
    ...
    >
      <div
        className='relative bg-[#0e1422] border border-zinc-800 rounded-lg shadow-2xl w-full max-w-4xl mx-4 flex flex-col'
          ...
          <div className='flex items-center gap-3'>
            ...
            <div>
              ...
              <p className='text-sm font-semibold text-zinc-500'>
                {mediaItems.length} assets in library
              </p>
            </div>
          </div>
          <button
            onClick={onClose}
            className='p-2 rounded-lg text-zinc-500 hover:text-white hover:bg-zinc-800 transition-all cursor-pointer'
          >
        ...
        <div className='flex-1 overflow-hidden flex flex-col min-h-0 px-6 py-4'>
                ...
                {loading ? (
                  ...
                  <div className='flex flex-col items-center justify-center h-48 gap-3 text-center'>
                    ...
                    {!search && (
                      <button
                        onClick={() => setTab('upload')}
                        className='px-4 py-2 bg-[#E61C24]/15 hover:bg-[#E61C24]/25 border border-[#E61C24]/20 text-[#E61C24] rounded-lg font-bold text-base transition-all cursor-pointer'
                      >
                        Go to Upload
                      </button>
                    )}
                      ...
                      return (
                        ...
                        >
                          <div className='aspect-square bg-[#0e1322] flex items-center justify-center overflow-hidden'>
                            ...
                            {/* Hover overlay */}
                            <div
                              className={`absolute inset-0 bg-[#E61C24]/20 flex items-center justify-center transition-opacity duration-150 ${isHov ? 'opacity-100' : 'opacity-0'}`}
                            >
          ...
          {tab === 'upload' && (
            <div className='flex flex-col gap-4 flex-1 min-h-0'>
              ...
              >
                <div
                  className={`h-16 w-16 rounded-full flex items-center justify-center transition-all ${dragOver ? 'bg-[#E61C24]/20 scale-110' : 'bg-zinc-800/50'}`}
                >
                  <FiUploadCloud
                    className={`h-8 w-8 transition-all ${dragOver ? 'text-[#E61C24]' : 'text-zinc-500'}`}
                  />
                </div>
                <div>
                ...
                <label
                  className={`inline-flex items-center gap-2 px-5 py-2.5 rounded-lg font-bold text-base transition-all cursor-pointer ${uploading ? 'opacity-50 pointer-events-none bg-zinc-800 text-zinc-500' : 'bg-[#E61C24] hover:bg-[#CC181F] text-white shadow-md shadow-[#E61C24]/20'}`}
                >
                  <input
                    ref={fileInputRef}
                    type='file'
                ...
                <p className='text-sm font-semibold text-zinc-600'>
                  PNG, JPEG, WEBP, GIF, PDF, MP4 supported
                </p>
              </div>
              ...
              {uploads.length > 0 && (
                ...
                >
                  ...
                  {uploads.map((u, i) => (
                    <div
                      key={i}
                      className={`flex items-center gap-3 px-4 py-3 rounded-lg border transition-all ${u.status === 'done' ? 'bg-emerald-500/8 border-emerald-500/20' : u.status === 'error' ? 'bg-rose-500/8 border-rose-500/20' : 'bg-[#121829] border-zinc-800'}`}
                    >
