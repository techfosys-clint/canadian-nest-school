          ...
          <div className='flex items-center gap-4'>
            <Link
              ...
              className={`relative p-2 transition-colors duration-200 group cursor-pointer flex items-center justify-center ${
                isScrolled
                  ? 'text-zinc-500 hover:text-zinc-900'
                  : 'text-[#0A163A]/70 hover:text-[#0A163A]'
              }`}
              aria-label={`Cart${itemCount > 0 ? `, ${itemCount} items` : ''}`}
            >
              <FiShoppingCart className='h-5 w-5 group-hover:scale-105 transition-transform' />
              {itemCount > 0 && (
                <span className='absolute -top-0.5 -right-0.5 min-w-4.5 h-4.5 px-1 flex items-center justify-center rounded-full bg-[#E61C24] text-white text-[11px] font-bold leading-none'>
            ...
            <div className='relative' ref={dropdownRef}>
              {loading ? (
                ...
                >
                  ...
                  <FiChevronDown
                    className={`h-4 w-4 text-zinc-400 group-hover:text-zinc-900 transition-transform duration-300 ${userMenuOpen ? 'rotate-180' : ''}`}
                  />
                </button>
              ) : (
                    ...
                    {user ? (
                      ...
                      <>
                        <div className='px-3 py-2.5 bg-zinc-50/70 rounded-lg mb-2'>
                          ...
                          <p className='text-base text-zinc-550 font-semibold truncate'>
                            {user.email}
                          </p>
                      ...
                      <>
                        <div className='px-3 py-2.5 bg-zinc-50/70 rounded-lg mb-3'>
                          ...
                          <p className='text-base text-zinc-550 font-semibold mt-0.5'>
                            Join Canadian Nest to access premium courses.
                          </p>
                        </div>
        ...
        {mobileMenuOpen && (
          <>
            ...
            >
              ...
              {/* Navigation Links */}
              <nav className='flex flex-col gap-2 p-6 grow overflow-y-auto'>
                <Link
                  href='/shop/cart'
                  onClick={() => setMobileMenuOpen(false)}
                ...
                >
                  <span>Cart</span>
                  {itemCount > 0 && (
                    <span className='min-w-5 h-5 px-1.5 flex items-center justify-center rounded-lg bg-[#E61C24] text-white text-sm font-bold'>
                      {itemCount > 99 ? '99+' : itemCount}
                    </span>
                  )}
                </Link>
                  ...
                  <div className='space-y-3'>
                    <div className='flex items-center gap-3 p-3 bg-white rounded-lg border border-zinc-200/60 shadow-xs'>
                      ...
                      <div className='min-w-0 grow'>
                        <p className='text-base font-bold text-zinc-800 truncate leading-snug'>
                          {user.name}
                        </p>
                        <p className='text-xs text-zinc-500 truncate leading-normal'>
                          {user.email}
                        </p>
                      </div>
                    </div>
